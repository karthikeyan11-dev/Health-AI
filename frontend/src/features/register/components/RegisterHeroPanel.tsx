...
import { REGISTER_TEXTS } from '../constants/register.constants';

export interface RegisterHeroPanelProps {
  onNavigateLogin: () => void;
}

export const RegisterHeroPanel: React.FC<RegisterHeroPanelProps> = ({ onNavigateLogin }) => {
  return (
    <div className="bg-gradient-primary text-white p-8 sm:p-12 lg:p-14 flex flex-col justify-between relative overflow-hidden">
      ...
      <div className="relative z-10 my-8 space-y-4">
        ...
        <div className="bg-white/15 backdrop-blur-md border border-white/20 p-4 rounded-2xl flex items-center gap-3.5 text-white shadow-sm mt-6">
          ...
          <div>
            <p className="text-base font-bold leading-tight">{REGISTER_TEXTS.badgeTitle}</p>
            <p className="text-xs text-white/80 leading-tight">{REGISTER_TEXTS.badgeSubtitle}</p>
          </div>
        </div>
      </div>
      ...
      <div className="relative z-10 space-y-3 pt-4 border-t border-white/15">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-xs font-semibold text-white/90">{REGISTER_TEXTS.switchPrompt}</p>
            <p className="text-[11px] text-white/70">{REGISTER_TEXTS.switchSubprompt}</p>
          </div>
          <Button
            type="button"
          ...
          >
            <span>{REGISTER_TEXTS.switchButtonText}</span>
